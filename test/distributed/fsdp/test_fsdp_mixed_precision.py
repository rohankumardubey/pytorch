...
class TestFSDPDifferentSubmodulePrecision(FSDPTest):
    ...
    def test_submodules_with_different_precisions_error(self):
        forward_inputs: Dict[nn.Module, torch.Tensor] = {}
        float16 = MixedPrecision(param_dtype=torch.float16, cast_forward_inputs=True)
        float8 = MixedPrecision(param_dtype=torch.float8, cast_forward_inputs=True)
        ...
        fsdp = FSDP(model, mixed_precision=float8)
        with self.assertRaisesRegex(
            RuntimeError, "mat1 and mat2 must have the same dtype"
        ):
            fsdp(x).sum().backward()
    ...
    def test_submodules_with_different_precisions(self):
        ...
        # For submodules with different precisions, it is recommended to wrap those
        # submodules seperatedly with ``cast_forward_inputs``=True, for root FSDP instance,
        # make sure its wrapped submodules are executed before other submodules,
        # it is OK to set ``cast_forward_inputs`` or ``cast_root_foward_inputs`` to be True
        model.c2 = FSDP(model.c2, mixed_precision=float16)
        fsdp = FSDP(model, mixed_precision=float8)

        fsdp(x).sum().backward()

        self.assertEqual(forward_inputs[model].dtype, torch.float8)
        self.assertEqual(forward_inputs[c1].dtype, torch.float8)
        self.assertEqual(forward_inputs[c2].dtype, torch.float16)

    @skip_if_lt_x_gpu(2)
    def test_submodules_with_external_inputs(self):
        class ToyModule(nn.modules):
